...
export class TripsService {
  ...
  async create(dto: CreateTripDto, currentUser: CurrentUserData) {
    ...
    const { companyId, route, vehicle } = await this.validateTripRelations(
      dto.routeId,
      dto.vehicleId,
      driverId,
      currentUser,
  ...
  async update(id: string, dto: UpdateTripDto, currentUser: CurrentUserData) {
    ...
    if (trip.status === TripStatus.COMPLETED) {
      throw new BadRequestException(
        'Không được cập nhật chuyến xe đã hoàn thành',
      );
    }
    ...
    if (nextTotalSeats < trip.bookedSeats) {
      throw new BadRequestException('Số ghế không được nhỏ hơn số ghế đã đặt');
    }
    ...
    trip.routeLineId = route.routeLineId;
    trip.direction = route.direction;
    ...
    if (dto.basePrice !== undefined) {
      trip.basePrice = dto.basePrice !== null ? String(dto.basePrice) : null;
    }
  ...
  async remove(id: string, currentUser: CurrentUserData) {
    ...
    if (trip.bookedSeats > 0) {
      throw new BadRequestException('Không được xóa chuyến xe đã có booking');
    }
